...
export function FlightCASLog({ points }: Props) {
  ...
  const activeEntries = timeline.filter(e => e.type !== "cleared");
  ...
  return (
    <div className="p-4 space-y-4" translate="no">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <span className="bg-amber-500/20 text-amber-400 border border-amber-500/30 px-3 py-1 rounded text-xs font-mono">
          {activeEntries.length} eventos
        </span>
      </div>
      ...
      <div className="bg-card rounded-md border border-border overflow-hidden">
        ...
        {/* Rows — only active events, verbatim from log */}
        <div className="divide-y divide-border">
          {activeEntries.map((entry, i) => (
            <div
              ...
              style={{ gridTemplateColumns: "90px 16px 1fr" }}
            >
              <span className="font-mono text-xs text-amber-400">{entry.time}</span>
              <span className={`w-2 h-2 rounded-full ${entry.type === "appeared" ? "bg-amber-400" : "bg-orange-400"}`} />
              <span className="font-mono text-xs text-foreground">{entry.message}</span>
            </div>
          ))}
        </div>
      </div>
